# calcium_dynamics_ui.py
# UI components for calcium dynamics configuration in Brian2Sim GUI
...
def create_calcium_dynamics_group(main_window):
    """Creates the 'Calcium Dynamics' group box using config-driven approach."""
    calcium_dynamics_group = QGroupBox("Calcium Dynamics")
    calcium_dynamics_group.setToolTip("Configure calcium dynamics and buffering systems.")

    # Create form generator
    main_window.calcium_dynamics_form_generator = CalciumDynamicsFormGenerator(
        CALCIUM_DYNAMICS_CONFIG
    )
    # Alias for simulation_manager compatibility
    main_window.calcium_dynamics_ui = main_window.calcium_dynamics_form_generator

    # Get the form widget and set it as the layout
    form_widget = main_window.calcium_dynamics_form_generator.get_form_widget()
    if form_widget:
        calcium_dynamics_group.setLayout(form_widget.layout())

        # Store references to specific widgets for backward compatibility
        param_widgets = main_window.calcium_dynamics_form_generator.get_param_widgets()
        main_window.calcium_dynamics_enabled = param_widgets.get("enabled")
            ...
            if param_key != "enabled":  # Don't connect the main enable checkbox
                if hasattr(widget, "valueChanged"):
                    widget.valueChanged.connect(
                        main_window.calcium_dynamics_manager.on_param_changed
                    )
    ...
    return calcium_dynamics_group


class CalciumDynamicsFormGenerator(BaseFormGenerator):
    """Form generator for calcium dynamics configuration with checkbox visibility pattern."""
    ...
    def _create_forms(self):
        ...
        if has_enabled_toggle:
            ...
            if enable_checkbox:
                ...
                default_enable_label = "Enable Calcium Dynamics"
        ...
        groups = {
            "General Parameters": {
                "keys": ["compartments", "ca_sources", "ca_extrusion_rate", "ca_rest"]
            },
            "Calcium Buffering": {
                "toggle": "ca_buffer_enabled",
                "keys": ["buffer_capacity", "buffer_kinetics"],
            },
        }
        ...
        for group_title, group_data in groups.items():
            ...
            if toggle_key and toggle_key in self.config:
                ...
                toggle_widget.setChecked(toggle_config.get("default", True))
