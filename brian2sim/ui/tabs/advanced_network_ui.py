...
from brian2sim.ui.ui_forms import AdvancedNetworkFormGenerator
...
def create_feature_tab(main_window, config_key, feature_name):
    ...
    if feature_name in ADVANCED_NETWORK_PRESETS and feature_name not in [
        "stdp",
...
def create_preset_section(main_window, feature_name, form_generator):
    ...
    preset_combo.currentIndexChanged.connect(
        lambda index: apply_preset_if_selected(
            main_window, feature_name, preset_combo, form_generator
        )
    )
...
def _store_widget_references(main_window, feature_name, param_widgets, form_generator):
    ...
    if feature_name == "dales_principle":
        ...
        if (
            hasattr(main_window, "advanced_preset_combos")
            and feature_name in main_window.advanced_preset_combos
        ):
