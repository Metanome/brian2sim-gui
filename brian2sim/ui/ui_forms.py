...
class BaseFormGenerator(QWidget):
    ...
    def _handle_initial_visibility(self):
        ...
        for form_key in self.forms.keys():
            current_config = (
                self.config.get(form_key)
                if isinstance(self.config.get(form_key), dict) and form_key in self.config
                else self.config
            )
    ...
    def _add_param_to_grid(self, param_key, grid_layout, row, col, param_widgets):
        """Helper to add parameter widget to a grid layout"""
        if param_key not in self.config:
            return False

        param_config_item = self.config[param_key]
        ...
        if input_widget:
            ...
            # Handle Checkbox layout (horizontal) vs others (vertical)
            ...
            # Store labels and cell widgets
            if "main" not in self.param_labels:
                self.param_labels["main"] = {}
            if "main" not in self.cell_widgets:
    ...
    def _setup_dependencies(self):
        ...
        Handles both simple form layouts and complex grid layouts with cell widgets.
        Uses compound AND logic for multiple depends_on conditions.
        """

        for form_key in self.forms.keys():
            param_widgets = self.get_param_widgets(form_key)
            param_labels = self.param_labels.get(form_key, {})
            # Support cell_widgets for grid layouts (used by SimParams, NoiseOptions, etc.)
            cell_widgets = getattr(self, "cell_widgets", {}).get(form_key, {})
            current_config = (
                self.config.get(form_key)
                if isinstance(self.config.get(form_key), dict) and form_key in self.config
                else self.config
            ...
            # Setup dependencies - create ONE handler per param that checks ALL conditions
            for param_key, param_config in current_config.items():
                if isinstance(param_config, dict) and param_config.get("depends_on"):
                    widget = param_widgets.get(param_key)
                    label = param_labels.get(param_key)
                    cell_widget = cell_widgets.get(param_key)  # May be None for simple forms
                    depends_on = param_config["depends_on"]

                    # Gather all control widgets for this param's dependencies
                    ctrl_widgets_info = []
                    for ctrl_param, expected_val in depends_on.items():
                        ctrl_widget = param_widgets.get(ctrl_param)
                        if ctrl_widget:
                            ctrl_widgets_info.append((ctrl_widget, expected_val))

                    if (widget or cell_widget) and ctrl_widgets_info:
                        # Create a single handler that checks ALL conditions (AND logic)
                        def make_compound_handler(
                            ...
                            def handler():
                                ...
                                # Handle cell widget visibility for grid layouts
                                if target_cell:
                                    grid_layout = (
                                        target_cell.parent().layout()
                                        if target_cell.parent()
                                        else None
                                    )
                                    if grid_layout and hasattr(grid_layout, "indexOf"):
                                        ...
                                        if index >= 0:
                                            row, col, row_span, col_span = (
                                                grid_layout.getItemPosition(index)
                                            )
                                            if show:
                                                target_cell.setVisible(True)
                                                # Make child widgets visible
                                                for child in target_cell.findChildren(QWidget):
                                                    child.setVisible(True)
                                                # Force grid layout update
                                                grid_layout.removeWidget(target_cell)
                                                grid_layout.addWidget(
                                                    target_cell, row, col, row_span, col_span
                                                )
                                            ...
                                            if target_cell.parent():
                                                target_cell.parent().updateGeometry()
                                        else:
                        ...
                        h = make_compound_handler(widget, label, cell_widget, ctrl_widgets_info)
                        self.dependency_handlers[form_key][param_key] = h

                        # Connect handler to ALL control widgets
                        for ctrl_widget, _ in ctrl_widgets_info:
...
class SimParamsFormGenerator(BaseFormGenerator):
    ...
    def _create_forms(self):
        ...
        for param_key, param_config_item in self.config.items():
            ...
            if input_widget:
                cell_widget = QWidget()
                actual_label_text = param_config_item.get(
                ...
                self.cell_widgets["main"][
                    param_key
                ] = cell_widget  # Store cell widget for dependency control

                if param_config_item.get("initially_hidden", False) or param_config_item.get(
                    "depends_on"
                ):
...
class AdvancedNetworkFormGenerator(BaseFormGenerator):
    ...
    def _create_forms(self):
        for section_key, section_config_dict in self.config.items():
            ...
            if has_enabled_toggle:
                ...
                section_enable_checkbox = self._create_widget_for_param(
                    f"{section_key}_enabled", enabled_config
                )
            ...
            param_configs_for_form = {
                k: v for k, v in section_config_dict.items() if k != "enabled"
            }
    ...
    def _update_stdp_parameters(self, param_widgets):
        ...
        from brian2sim.models.advanced_network_config import STDP_TYPE_DEFAULTS
    ...
    class MockFormGenerator:
        ...
        def _create_widget_for_param(self, param_key, param_config):
            ...
            if param_type == "bool" or param_type == "checkbox":
                ...
                if default_value is not None and default_value in options:
                    input_widget.setCurrentIndex(options.index(default_value))
            elif param_config.get("widget") == "text_area" or (
                param_type == str and param_key == "custom_eqs"
            ):
                input_widget = QTextEdit()
                input_widget.setMinimumHeight(80)
                if default_value is not None:
    ...
    def _create_forms(self):
        ...
        if has_enabled_toggle:
            ...
            if enable_checkbox:
                ...
                default_enable_label = "Enable Input Patterns"
                enable_label_text = enabled_config.get("label", default_enable_label)
                enable_label = QLabel(enable_label_text)
                enable_layout.addWidget(enable_label)
                enable_layout.addStretch()
                ...
                # We need to define params_group_widget before connecting signals
                self.params_group_widget = QWidget()
