...
class SimParamsManager(QObject):
    ...
    def connect_lif_signals(self):
        ...
        if (
            hasattr(self.main_window, "lif_threshold_input")
            and self.main_window.lif_threshold_input is not None
        ):
        ...
        if (
            hasattr(self.main_window, "lif_reset_input")
            and self.main_window.lif_reset_input is not None
        ):
            # Disconnect any existing connection to avoid duplicates
            try:
    ...
    def get_sim_params(self):
        ...
        if hasattr(self.main_window, "sim_params_form_generator"):
            ...
            if (
                hasattr(self.main_window, "neuron_model_combo")
                and self.main_window.neuron_model_combo.currentData() == "lif"
            ):
                if (
                    hasattr(self.main_window, "lif_threshold_input")
                    and self.main_window.lif_threshold_input
                ):
                    params["lif_threshold"] = self.main_window.lif_threshold_input.value()
                if (
                    hasattr(self.main_window, "lif_reset_input")
                    and self.main_window.lif_reset_input
                ):
    ...
    def load_sim_params(self, data):
        ...
        if hasattr(self.main_window, "sim_params_form_generator"):
            ...
            if (
                hasattr(self.main_window, "input_current_input")
                and self.main_window.input_current_input
            ):
                self.main_window.input_current_input.setValue(data.get("input_current", 150.0))

            if (
                hasattr(self.main_window, "num_neurons_input")
                and self.main_window.num_neurons_input
            ):
                self.main_window.num_neurons_input.setValue(data.get("num_neurons", 100))

            if (
                hasattr(self.main_window, "current_start_input")
                and self.main_window.current_start_input
            ):
                self.main_window.current_start_input.setValue(data.get("current_start", 100))

            if (
                hasattr(self.main_window, "current_duration_input")
                and self.main_window.current_duration_input
            ):
                self.main_window.current_duration_input.setValue(data.get("current_duration", 500))

            # Load LIF parameters if available
            if (
                "lif_threshold" in data
                and hasattr(self.main_window, "lif_threshold_input")
                and self.main_window.lif_threshold_input
            ):
                self.main_window.lif_threshold_input.setValue(data["lif_threshold"])
            if (
                "lif_reset" in data
                and hasattr(self.main_window, "lif_reset_input")
                and self.main_window.lif_reset_input
            ):
    ...
    def update_lif_params_visibility(self, model_key):
        ...
        if hasattr(self.main_window, "sim_params_form_generator"):
            ...
            if (
                hasattr(self.main_window, "sim_params_ui_module")
                and self.main_window.sim_params_ui_module
            ):
                self.main_window.sim_params_ui_module.update_lif_parameters_visibility_in_sim_params(
                    self.main_window, model_key
                )
    ...
    def apply_preset_values(self, preset_values):
        ...
        if hasattr(self.main_window, "sim_params_form_generator"):
            ...
            # Fallback to legacy approach
            if (
                "sim_time" in preset_values
                and hasattr(self.main_window, "sim_time_input")
                and self.main_window.sim_time_input
            ):
                self.main_window.sim_time_input.setValue(preset_values["sim_time"])
            if (
                "input_current" in preset_values
                and hasattr(self.main_window, "input_current_input")
                and self.main_window.input_current_input
            ):
                self.main_window.input_current_input.setValue(preset_values["input_current"])
            if (
                "num_neurons" in preset_values
                and hasattr(self.main_window, "num_neurons_input")
                and self.main_window.num_neurons_input
            ):
                self.main_window.num_neurons_input.setValue(preset_values["num_neurons"])
            if (
                "current_start" in preset_values
                and hasattr(self.main_window, "current_start_input")
                and self.main_window.current_start_input
            ):
                self.main_window.current_start_input.setValue(preset_values["current_start"])
            if (
                "current_duration" in preset_values
                and hasattr(self.main_window, "current_duration_input")
                and self.main_window.current_duration_input
            ):
                self.main_window.current_duration_input.setValue(preset_values["current_duration"])
            if (
                "lif_threshold" in preset_values
                and hasattr(self.main_window, "lif_threshold_input")
                and self.main_window.lif_threshold_input
            ):
                self.main_window.lif_threshold_input.setValue(preset_values["lif_threshold"])
            if (
                "lif_reset" in preset_values
                and hasattr(self.main_window, "lif_reset_input")
                and self.main_window.lif_reset_input
            ):
