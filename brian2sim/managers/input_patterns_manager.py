        ...
        if hasattr(self.main_window, "input_patterns_forms"):
            ...
            if enabled_widget:
                ...
                enabled_widget.toggled.connect(
                    lambda x: self.param_changed.emit("input_patterns_enabled", x)
                )
            ...
            if pattern_type_widget:
                ...
                pattern_type_widget.currentTextChanged.connect(
                    lambda x: self.param_changed.emit("input_pattern_type", x)
                )
                ...
                if param_name not in ["enabled", "pattern_type"] and widget:
                    if hasattr(widget, "valueChanged"):  # Spin boxes
                        widget.valueChanged.connect(
                            lambda val, name=param_name: self.param_changed.emit(name, val)
                        )
