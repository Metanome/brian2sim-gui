...
from brian2sim.models.neuron_models_config import (
...
class NeuronModelsManager:
    ...
    def connect_signals(self):
        self.main_window.neuron_model_combo.currentIndexChanged.connect(
            self.update_neuron_param_form_and_presets
        )
        self.main_window.neuron_model_preset_combo.currentIndexChanged.connect(
            self.apply_neuron_model_preset
        )
        ...
        self.main_window.noise_manager.param_changed.connect(self.on_param_changed)
        self.main_window.network_manager.param_changed.connect(self.on_param_changed)
    ...
    def update_neuron_param_form_and_presets(self, index):
        ...
        if (
            not model_key
            or not self.main_window.neuron_model_stacked_widget
            or not self.main_window.neuron_model_forms
        ):
            return
        ...
        if "presets" in model_config:
            for preset_key, preset in model_config["presets"].items():
                if preset_key not in ["none", "custom"]:  # Skip the special presets
                    self.main_window.neuron_model_preset_combo.addItem(
                        preset["display_name"], userData=preset_key
                    )
    ...
    def apply_neuron_model_preset(self, index):
        if not (
            ...
            and self.main_window.neuron_model_preset_combo
            and self.main_window.neuron_model_forms
        ):
            return
            ...
            if model_config and "presets" in model_config:
                ...
                if preset and "values" in preset:
                    ...
                    if current_model_key in self.main_window.neuron_model_forms:
                        param_widgets = self.main_window.neuron_model_forms[current_model_key][
                            "params"
                        ]
                    ...
                    if "sim_params" in preset:
                        self.main_window.sim_params_manager.apply_preset_values(
                            preset["sim_params"]
                        )
                    if "noise" in preset:
                        self.main_window.noise_manager.apply_preset_values(
                            preset.get("noise", {})
                        )
                    if "network" in preset:
                        self.main_window.network_manager.apply_preset_values(
                            preset.get("network", {})
                        )
        finally:
    ...
    def get_neuron_model_config(self):
        ...
        config = {"model_key": model_key, "model_name": current_model_name, "parameters": {}}
    ...
    def load_neuron_model_config(self, neuron_settings):
        ...
        if model_key in self.main_window.neuron_model_forms:
            self.main_window.neuron_model_combo.setCurrentText(
                NEURON_MODELS_CONFIG[model_key]["display_name"]
            )
            ...
            try:
                ...
                if "preset" in neuron_settings:
                    ...
                    preset_key = None
                    model_presets = NEURON_MODELS_CONFIG.get(model_key, {}).get("presets", {})
                    for key, preset in model_presets.items():
                        if preset.get("display_name") == preset_name:
                            preset_key = key
                            break
                    if preset_key:
    ...
    def update_neuron_model_display_name(self, new_name):
        ...
        if model_key:
            ...
            self.main_window.neuron_model_combo.setItemText(
                self.main_window.neuron_model_combo.currentIndex(), new_name
            )
    ...
    def add_neuron_model(self, model_key, model_config):
        ...
        self.main_window.neuron_model_combo.addItem(
            model_config["display_name"], userData=model_key
        )
    ...
    def update_neuron_model_param(self, model_key, param_name, value):
        ...
        if "params" in config and param_name in config["params"]:
            ...
            raise ValueError(
                f"Parameter '{param_name}' does not exist for model key '{model_key}'."
            )
    ...
    def load_neuron_models_config(self, file_path):
        ...
        MODEL_PRESETS = {
            model_key: config["presets"] for model_key, config in NEURON_MODELS_CONFIG.items()
        }
    ...
    def reset_to_defaults(self):
        """Reset neuron model to default (first model, default preset)."""
        # Reset to first neuron model
        if hasattr(self.main_window, "neuron_model_combo"):
            self.main_window.neuron_model_combo.setCurrentIndex(0)
        
        # Reset preset to default
        if hasattr(self.main_window, "neuron_model_preset_combo"):
            self.main_window.neuron_model_preset_combo.setCurrentIndex(0)
        
        # Reset parameters for the current model to defaults
        model_key = self.main_window.neuron_model_combo.currentData()
        if model_key and model_key in NEURON_MODELS_CONFIG:
            model_config = NEURON_MODELS_CONFIG[model_key]
            if model_key in self.main_window.neuron_model_forms:
                param_widgets = self.main_window.neuron_model_forms[model_key]["params"]
                for param_name, widget in param_widgets.items():
                    param_config = model_config.get("params", {}).get(param_name, {})
