...
class AdvancedNetworkManager(QObject):
    ...
    def toggle_stdp_visibility(self, state):
        ...
        if hasattr(self.main_window, 'stdp_params_group'):
            self.main_window.stdp_params_group.setVisible(
                state == Qt.CheckState.Checked.value
            )
    ...
    def update_distance_params_visibility(self, index):
        ...
        function_widget = self.main_window.distance_connectivity_forms['params'].get('connection_function')
        if not function_widget:
            return
            
        # Get actual function type value from combo box
        function_type_config = self._get_param_config('distance_connectivity', 'connection_function')
        ...
        for param_key in self.main_window.distance_connectivity_forms['params'].keys():
            if param_key not in ['connection_function', 'enabled', 'spatial_layout']:
                ...
                if param_key in self.main_window.distance_connectivity_forms['params']:
                    self.main_window.distance_connectivity_forms['params'][param_key].setVisible(is_visible)
                
                # Hide/show the parameter label
                if param_key in self.main_window.distance_connectivity_forms['labels']:
                    self.main_window.distance_connectivity_forms['labels'][param_key].setVisible(is_visible)
    ...
    def get_advanced_network_options(self):
        ...
        # Dale's Principle
        if hasattr(self.main_window, 'dales_principle_checkbox') and \
           self.main_window.dales_principle_checkbox.isChecked():
            dales_options = {"enabled": True}
            if hasattr(self.main_window, 'dales_principle_forms') and self.main_window.dales_principle_forms:
                dales_options.update(self._collect_feature_params('dales_principle', self.main_window.dales_principle_forms))
            options["dales_principle"] = dales_options
        else:
            options["dales_principle"] = {"enabled": False}

        # Synaptic Delays
        if hasattr(self.main_window, 'synaptic_delays_checkbox') and \
           self.main_window.synaptic_delays_checkbox.isChecked():
            delays_options = {"enabled": True}
            if hasattr(self.main_window, 'synaptic_delays_forms') and self.main_window.synaptic_delays_forms:
                delays_options.update(self._collect_feature_params('synaptic_delays', self.main_window.synaptic_delays_forms))
            options["synaptic_delays"] = delays_options
        else:
            options["synaptic_delays"] = {"enabled": False}

        # STDP
        if hasattr(self.main_window, 'stdp_checkbox') and \
           self.main_window.stdp_checkbox.isChecked():
            stdp_options = {"enabled": True}
            if hasattr(self.main_window, 'stdp_forms') and self.main_window.stdp_forms:
                stdp_options.update(self._collect_feature_params('stdp', self.main_window.stdp_forms))
            options["stdp"] = stdp_options
        else:
            options["stdp"] = {"enabled": False}

        # Distance-Dependent Connectivity
        if hasattr(self.main_window, 'distance_connectivity_checkbox') and \
           self.main_window.distance_connectivity_checkbox.isChecked():
            ...
            if hasattr(self.main_window, 'distance_connectivity_forms') and self.main_window.distance_connectivity_forms:
                distance_options.update(self._collect_feature_params('distance_connectivity', self.main_window.distance_connectivity_forms))
    ...
    def load_advanced_network_options(self, data):
        ...
        if hasattr(self.main_window, 'distance_connectivity_checkbox'):
            self.main_window.distance_connectivity_checkbox.setChecked(distance_data.get("enabled", False))
            
        if distance_data.get("enabled", False) and hasattr(self.main_window, 'distance_connectivity_forms'):
            ...
            if param_key in data:
                try:
                    if hasattr(widget, 'setValue'):
                        ...
                        param_config = None
                        if hasattr(self.main_window, 'synaptic_delays_forms') and forms == self.main_window.synaptic_delays_forms:
                            param_config = SYNAPTIC_DELAYS_CONFIG.get(param_key)
                        elif hasattr(self.main_window, 'stdp_forms') and forms == self.main_window.stdp_forms:
                            param_config = STDP_CONFIG.get(param_key)
                        elif hasattr(self.main_window, 'distance_connectivity_forms') and forms == self.main_window.distance_connectivity_forms:
                            param_config = DISTANCE_CONNECTIVITY_CONFIG.get(param_key)
                        
                        if param_config and param_config.get("type") == "combo" and "display_options" in param_config:
    ...
    def apply_advanced_preset(self, preset_name):
        ...
        if "dales_principle" in preset:
            ...
            if hasattr(self.main_window, 'dales_principle_checkbox'):
                self.main_window.dales_principle_checkbox.setChecked(dales_preset.get("enabled", False))
            if dales_preset.get("enabled", False) and hasattr(self.main_window, 'dales_principle_forms'):
        ...
        if "synaptic_delays" in preset:
            ...
            if hasattr(self.main_window, 'synaptic_delays_checkbox'):
                self.main_window.synaptic_delays_checkbox.setChecked(delays_preset.get("enabled", False))
            if delays_preset.get("enabled", False) and hasattr(self.main_window, 'synaptic_delays_forms'):
        ...
        if "distance_connectivity" in preset:
            ...
            if hasattr(self.main_window, 'distance_connectivity_checkbox'):
                self.main_window.distance_connectivity_checkbox.setChecked(distance_preset.get("enabled", False))
            if distance_preset.get("enabled", False) and hasattr(self.main_window, 'distance_connectivity_forms'):
                self._load_feature_params(self.main_window.distance_connectivity_forms, distance_preset)
    ...
    def apply_distance_connectivity_preset(self, preset_name):
        ...
        if hasattr(self.main_window, 'distance_connectivity_forms') and 'params' in preset:
            self._load_feature_params(self.main_window.distance_connectivity_forms, preset['params'])
