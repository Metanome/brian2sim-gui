...
from multicompartment_config import MULTICOMPARTMENT_CONFIG

def create_multicompartment_group(main_window):
    multicompartment_group = QGroupBox("Multi-Compartment")
    multicompartment_group.setToolTip("Configure multi-compartment neuron models.")
    main_window.multicompartment_form_generator = MulticompartmentFormGenerator(MULTICOMPARTMENT_CONFIG)
    form_widget = main_window.multicompartment_form_generator.get_form_widget()
    if form_widget:
        multicompartment_group.setLayout(form_widget.layout())
        param_widgets = main_window.multicompartment_form_generator.get_param_widgets()
        main_window.multicompartment_enabled = param_widgets.get("enabled")
        for param_key, widget in param_widgets.items():
            if param_key != "enabled":
                if hasattr(widget, 'valueChanged'):
                    widget.valueChanged.connect(main_window.multicompartment_manager.on_param_changed)
                elif hasattr(widget, 'currentIndexChanged'):
                    widget.currentIndexChanged.connect(main_window.multicompartment_manager.on_param_changed)
    return multicompartment_group

class MulticompartmentFormGenerator(BaseFormGenerator):
    """Form generator for multicompartment configuration with checkbox visibility pattern."""
    ...
    def _create_forms(self):
        ...
        if has_enabled_toggle:
            ...
            if enable_checkbox:
                ...
                default_enable_label = "Enable Multi-compartment Models"
