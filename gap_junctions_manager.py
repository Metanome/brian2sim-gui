...
from gap_junctions_config import GAP_JUNCTIONS_CONFIG, GAP_JUNCTION_RESEARCH_INFO
...
class GapJunctionsManager(QObject):
    ...
    def get_coupling_strength_estimate(self, params):
        ...
        if params.get('activity_dependent', False):
            plasticity_factor = params.get('potentiation_rate', 0.01) / params.get('depression_rate', 0.001)
    ...
    def generate_network_analysis_suggestions(self, params):
        ...
        if params.get('activity_dependent', False):
            suggestions.append("Activity-dependent plasticity enabled - track coupling changes over time")
            suggestions.append("Monitor plasticity threshold crossings")
        
        if params.get('voltage_dependence', False):
            suggestions.append("Voltage-dependent gating enabled - analyze voltage-coupling relationships")
        
        if params.get('spatial_organization') == 'columnar':
            suggestions.append("Columnar organization - analyze within-column vs between-column coupling")
