...
from ui_forms import NetworkOptionsFormGenerator
        ...
        for param_key, widget in param_widgets.items():
            if param_key != "enabled":  # Don't connect the main enable checkbox
                if hasattr(widget, 'valueChanged'):
                    widget.valueChanged.connect(main_window.network_options_manager.on_param_changed)
                elif hasattr(widget, 'currentIndexChanged'):
                    widget.currentIndexChanged.connect(main_window.network_options_manager.on_param_changed)
