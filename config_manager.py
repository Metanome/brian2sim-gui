...
class ConfigManager:
    ...
    def save_config(self, config_data, file_path):
        ...
        if not is_valid:
            QMessageBox.warning(self.main_window, "Invalid Configuration", 
        ...
        try:
            ...
            QMessageBox.critical(self.main_window, "Error", f"An unexpected error occurred during save: {e}")
    ...
    def load_config(self, file_path):
        ...
        try:
            ...
            QMessageBox.warning(self.main_window, "Error", "Invalid JSON file. Could not load configuration.")
            return None
        except Exception as e:
            QMessageBox.critical(self.main_window, "Error", f"An unexpected error occurred during load: {e}")
            return None
    ...
    def validate_parameters(self, config_data):
        ...
        if not isinstance(current_duration, (int, float)) or not (0 <= current_duration <= sim_time):
            ...
            if "lif_reset" in sim_params:
                ...
                if "lif_threshold" in sim_params:
                    ...
                    if lif_reset >= lif_threshold:
                        return False, f"LIF reset ({lif_reset} mV) must be less than threshold ({lif_threshold} mV)"
    ...
    def _validate_adex_parameters(self, neuron_settings):
        ...
        try:
            ...
            if not (-80.0 <= EL <= -50.0):
                return False, "Parameter 'EL' (leak reversal potential) must be between -80 and -50 mV"
            if not (-60.0 <= VT <= -40.0):
            ...
            if not (1.0 <= tauw <= 500.0):
                return False, "Parameter 'tauw' (adaptation time constant) must be between 1 and 500 ms"
            if not (0.0 <= b <= 500.0):
                return False, "Parameter 'b' (spike-triggered adaptation) must be between 0 and 500 pA"
        except (TypeError, ValueError):
    ...
    def _validate_advanced_network(self, advanced_options):
        ...
        if "dales_principle" in advanced_options:
            ...
            if "exc_reversal" in dales:
                ...
                if not isinstance(exc_reversal, (int, float)) or not (-20.0 <= exc_reversal <= 20.0):
            ...
            if "inh_reversal" in dales:
                ...
                if not isinstance(inh_reversal, (int, float)) or not (-100.0 <= inh_reversal <= -50.0):
        ...
        if "distance_connectivity" in advanced_options:
            ...
            if "connection_function" in distance:
                ...
                if func not in valid_functions:
                    return False, f"Connection function must be one of: {', '.join(valid_functions)}"
