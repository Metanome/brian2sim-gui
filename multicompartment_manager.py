...
class MulticompartmentManager(QObject):
    ...
    def validate_parameters(self, params: Dict) -> Tuple[bool, List[str]]:
        ...
        if params.get('channel_distribution', True):
            ...
            if nav_ais < nav_soma:
                warnings.append("AIS Nav density lower than soma - spike initiation may be compromised")
    ...
    def suggest_parameters(self, cell_type: str, brain_region: str, species: str, age: str) -> Dict:
        ...
        species_scaling = self.morphology_presets['species_scaling'].get(species, 
                                                                        self.morphology_presets['species_scaling']['rat'])
        ...
        dev_scaling = self.morphology_presets['developmental_scaling'].get(age,
                                                                           self.morphology_presets['developmental_scaling']['adult'])
        ...
        if 'soma_diameter' in base_params:
            ...
            for channel_param in ['soma_nav_density', 'ais_nav_density', 'dendrite_nav_density',
                                 'soma_kv_density', 'ais_kv_density', 'dendrite_kv_density']:
        ...
        if 'dendrite_complexity' in region_mods:
            if 'dendrite_segments' in base_params:
                base_params['dendrite_segments'] = int(base_params['dendrite_segments'] * region_mods['dendrite_complexity'])
            if 'branch_points' in base_params:
                base_params['branch_points'] = max(1, int(base_params['branch_points'] * region_mods['dendrite_complexity']))
    ...
    def calculate_electrical_properties(self, params: Dict) -> Dict:
        ...
        dend_segments = params.get('dendrite_segments', 10)
    ...
    def generate_morphology_summary(self, params: Dict) -> str:
        ...
        summary.append(f"Estimated input resistance: {calc_props['input_resistance_estimate_MOhm']:.1f} MΩ")
