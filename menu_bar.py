...
from PyQt6.QtWidgets import QMessageBox, QFileDialog, QDialog, QVBoxLayout, QTextEdit, QHBoxLayout, QPushButton
...
class MenuBarManager(QObject):
    ...
    def setup_menu_bar(self):
        ...
        user_level_menu.addAction('Beginner (Core + Simulation)', lambda: self.set_user_level('beginner'))
        user_level_menu.addAction('Intermediate (+ Network + Synapses)', lambda: self.set_user_level('intermediate'))
        user_level_menu.addAction('Advanced (All Features)', lambda: self.set_user_level('advanced'))
        ...
        view_menu.addAction('Simulation Tab (Always Visible)', lambda: self.show_tab_info('simulation'))
        view_menu.addSeparator()
        ...
        view_menu.addAction('Toggle Plasticity Tab', lambda: self.toggle_tab_visibility('plasticity'))
        view_menu.addAction('Toggle Neuromodulation Tab', lambda: self.toggle_tab_visibility('neuromodulation'))
        view_menu.addAction('Toggle Multi-Compartment Tab', lambda: self.toggle_tab_visibility('multicompartment'))
    ...
    def new_simulation(self):
        ...
        reply = QMessageBox.question(
            self.main_window, 'New Simulation', 
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                for manager_name in ['gap_junctions_manager', 'synaptic_receptors_manager',
                                   'calcium_dynamics_manager', 'short_term_plasticity_manager',
                                   'homeostatic_plasticity_manager', 'neuromodulation_manager',
                ...
                QMessageBox.information(self.main_window, 'New Simulation', 
                                      'All parameters have been reset to defaults.')
                self.new_simulation_requested.emit()
                
            except Exception as e:
                QMessageBox.warning(self.main_window, 'Reset Error', 
                                  f'Error resetting parameters: {str(e)}')
    
    def show_validation_dialog(self):
        """Show a comprehensive validation results dialog."""
        validation_results = self.main_window.validation_manager.validate_all_parameters()
        
        if not validation_results:
            QMessageBox.information(self.main_window, 'Validation Results', 
        ...
        results_text += f"Errors: {len(errors)} | Warnings: {len(warnings)} | Info: {len(infos)}\n\n"
    ...
    def export_validation_report(self, validation_results):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self.main_window, 'Export Validation Report', 
        ...
        if filename:
            try:
                with open(filename, 'w') as f:
                    ...
                    f.write(f"Errors: {len(errors)} | Warnings: {len(warnings)} | Info: {len(infos)}\n\n")
                ...
                QMessageBox.warning(self.main_window, 'Export Error', 
                    f'Failed to export report:\n{str(e)}')
        
    def export_config(self):
        """Export current configuration to a file."""
        filename, _ = QFileDialog.getSaveFileName(
            self.main_window, 'Export Configuration', 
    ...
    def generate_code(self):
        ...
        try:
            ...
            if hasattr(self.main_window, 'code_generator'):
                ...
                QMessageBox.information(self.main_window, 'Code Generation', 
                                      'Code generator not available.')
        except Exception as e:
            QMessageBox.warning(self.main_window, 'Code Generation Error', 
                              f'Failed to generate code:\n{str(e)}')
    
    def save_generated_code(self, code):
        """Save generated code to a file."""
        filename, _ = QFileDialog.getSaveFileName(
            self.main_window, 'Save Generated Code', 
        ...
        if filename:
            try:
                with open(filename, 'w') as f:
                    f.write(code)
                QMessageBox.information(self.main_window, 'Save Complete', 
                    f'Code saved to:\n{filename}')
            except Exception as e:
                QMessageBox.warning(self.main_window, 'Save Error', 
                    f'Failed to save code:\n{str(e)}')
    ...
    def copy_to_clipboard(self, text):
        ...
        QMessageBox.information(self.main_window, 'Copied', 
    ...
    def toggle_tab_visibility(self, tab_key):
        ...
        if hasattr(self.main_window, 'tab_info') and tab_key in self.main_window.tab_info:
            ...
            QMessageBox.information(
                ...
                f'{tab_name} tab has been {status}.'
            )
