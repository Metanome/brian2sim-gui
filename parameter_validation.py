...
class ParameterValidator:
    ...
    def setup_default_rules(self):
        ...
        self.add_range_rule("voltage", -100, 50, "mV", 
                           "Membrane potential should be between -100mV and +50mV")
        
        # Time constant rules
        self.add_range_rule("tau", 0.1, 1000, "ms",
                           "Time constants should be between 0.1ms and 1000ms")
        
        # Conductance rules
        self.add_range_rule("conductance", 0, 1000, "nS",
                           "Conductances should be between 0 and 1000 nS")
        
        # Probability rules
        self.add_range_rule("probability", 0, 1, "",
                           "Probabilities must be between 0 and 1")
        
        # Concentration rules
        self.add_range_rule("concentration", 0, 1000, "mM",
                           "Concentrations should be between 0 and 1000 mM")
        
        # Setup cross-parameter validation rules
        self.setup_cross_parameter_rules()
    
    def add_range_rule(self, param_pattern: str, min_val: float, max_val: float, 
    ...
    def _validate_network_consistency(self, params: Dict[str, Any]) -> ValidationResult:
        ...
        if exc_prob > 0.5 or inh_prob > 0.5:
            return ValidationResult(True, "High connection probabilities may create dense networks", "warning")
    ...
    def validate_parameter(self, param_name: str, value: Any) -> ValidationResult:
        ...
        if isinstance(value, (int, float)):
            if value < 0 and any(keyword in param_name.lower() 
                               for keyword in ['time', 'delay', 'duration', 'rate']):
    ...
    def _validate_cross_parameter_rule(self, rule: Dict, params: Dict[str, Any]) -> ValidationResult:
...
class ValidationManager(QObject):
    ...
    def show_validation_dialog(self) -> bool:
        ...
        if errors:
            ...
            QMessageBox.critical(self.main_window, "Parameter Validation Errors", 
                               "The following errors must be fixed before proceeding:\\n\\n" + dialog_text)
            return False
        else:
            # Only warnings - user can choose to continue
            reply = QMessageBox.question(self.main_window, "Parameter Validation Warnings",
